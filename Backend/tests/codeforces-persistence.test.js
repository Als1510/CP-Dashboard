// Codeforces persistence integration tests (focused, stubbed)
...
(async () => {
  // T1 — stored GET
  stub.store = {};
  stub.store['codeforces:tourist'] = { platform:'codeforces', username:'tourist', payload_json:{status:'OK'}, last_fetched:new Date(), created_at:new Date() };
  const t1 = await stub.get('codeforces','tourist');
  assert.strictEqual(t1 && t1.payload_json.status, 'OK', 'T1 stored GET');
  console.log('PASS T1 — Stored GET');

  // T2 — cache miss → fetch + persist
  stub.store = {};
  assert.strictEqual(await stub.get('codeforces','tourist'), null, 'T2 miss');
  await stub.save('codeforces','tourist',{status:'OK', rating:2000});
  const t2 = await stub.get('codeforces','tourist');
  ...
  stub.store = {};
  await stub.save('codeforces','tourist',{rating:1900});
  const before = await stub.get('codeforces','tourist');
  await stub.save('codeforces','tourist',{rating:2050});
  const after = await stub.get('codeforces','tourist');
  ...
  stub.store = {};
  await stub.save('codeforces','tourist',{rating:1900});
  const old = await stub.get('codeforces','tourist');
  // failure path: do NOT call save → unchanged (demonstrated by not calling)
  assert.deepStrictEqual((await stub.get('codeforces','tourist')).payload_json, old.payload_json, 'T4 unchanged');
  console.log('PASS T4 — Refresh failure preserves old');

  // T5 — first failure, no record
  stub.store = {};
  assert.strictEqual(await stub.get('codeforces','noone'), null, 'T5 no record');
  console.log('PASS T5 — First fetch failure no record');

  console.log('All Codeforces persistence tests passed (stubbed, no live DB required).');
})();